...
<https://leetcode-cn.com/problems/construct-binary-tree-from-inorder-and-postorder-traversal/>
    ...
    public TreeNode buildTree(int[] inorder, int inS, int inE, int[] postorder, int poS, int poE){
        ...
        root.right=buildTree(inorder, index+1, inE, postorder, poS+leftSize, poE-1);
        return root;
    }
...
## O(lgn)
    ...
    public int countNodes(TreeNode root) {
        ...
        if(l==r){ //左右子树的高度相等时为满二叉树，节点数量为 pow(2, height)-1
            return (1<<l) -1;
        }else{
...
<https://leetcode-cn.com/problems/find-duplicate-subtrees/>
    ...
    public String track(TreeNode node){
        if(node==null) return "#";
        String left = track(node.left);
        String right = track(node.right);
        String s = node.val + "," + left + "," + right;
        frequency.put(s, frequency.getOrDefault(s, 0)+1);
        if(frequency.get(s)==2) res.add(node);
        return s;
    }
